        ...
        if (!$users->isEmpty()) {
            ...
            if ($n > 0) {
                ...
                foreach ($users as $user) {
                    ...
                    // dice 3D
                        ...
                        $gameService
                            ->play([
                                ...
                                'ref_number'    => random_int(\Packages\GameDice3D\Services\GameDice3DService::calcMinRefNumber(), \Packages\GameDice3D\Services\GameDice3DService::calcMaxRefNumber())
                            ]);
                    // heads or tails
                    ...
                    } elseif ($gameServiceClass == 'Packages\GameHorseRacing\Services\GameHorseRacingService') {
                        $runnersCount = count(config('game-horse-racing.runners'));
                        $betsCount = random_int(1, $runnersCount);
                        $bets = [];

                        for ($i = 0; $i < $betsCount; $i++) {
                            $bets[] = [
                                'type'      => array_rand([\Packages\GameHorseRacing\Helpers\RaceBet::BET_WIN, \Packages\GameHorseRacing\Helpers\RaceBet::BET_PLACE, \Packages\GameHorseRacing\Helpers\RaceBet::BET_SHOW]),
                                'bet'       => random_int($minBet ?: config('game-horse-racing.min_bet'), $maxBet ?: config('game-horse-racing.max_bet')),
