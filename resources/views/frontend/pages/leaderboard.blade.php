...
@section('content')
    <div class="btn-group mb-4" role="group" aria-label="{{ __('All games') }}">
        ...
        @elseif(Request::get('game') == 'HorseRacing')
            <a href="{{ route('frontend.leaderboard', array_merge(request()->query(), ['HorseRacing' => 'Keno'])) }}" class="btn btn-primary">{{ __('Horse Racing') }}</a>
        @else
        ...
        <div class="dropdown-menu">
            ...
            @endinstalled
            @installed('game-horse-racing')
            <a class="dropdown-item" href="{{ route('frontend.leaderboard', array_merge(request()->query(), ['game' => 'HorseRacing'])) }}">
                {{ __('Horse Racing') }}
            </a>
            @endinstalled
        </div>
